...
class ReaderTest(TestCase):
    ...
    def compare_lines(self, lines):
        for i, line in enumerate(lines):
            for k in line:
                self.assertEquals(
                    self.f_data[i][k], line[k],
                    "Line %s, %s failed: %s != %s." % (
                        i, k, repr(self.f_data[i][k]), repr(line[k])))
...
class ImportersValidationsTests(TestCase):
    ...
    def test_invalid_errors_in_logging_to_DB(self):
        ...
        self.assertTrue(
            u"SimpleValidationsImporterDB_importer :: error :: Line 2, field cpf: Invalid CPF number." in
            str(errors[0]), u"Weird string for this test, check loggers.")
        self.assertTrue(
            u"SimpleValidationsImporterDB_importer :: error :: Line 3, field cpf: Invalid CPF number." in
    ...
    def test_invalid_required_field_DB(self):
        ...
        self.assertTrue(
            u"RequiredFieldValidationsImporterDB_importer :: error :: Line 1, field cpf: Field cpf is required!" in
            str(errors[0]), u"Weird string for this test, check loggers.")
        self.assertTrue(
            u"RequiredFieldValidationsImporterDB_importer :: error :: Line 1, field field3: Field field3 is required!" in
            str(errors[1]), u"Weird string for this test, check loggers.")
        self.assertTrue(
            u"RequiredFieldValidationsImporterDB_importer :: error :: Line 2, field cpf: Invalid CPF number." in
            str(errors[2]), u"Weird string for this test, check loggers.")
        self.assertTrue(
            u"RequiredFieldValidationsImporterDB_importer :: error :: Line 3, field cpf: Invalid CPF number." in
